...
class EmailTemplateRendererTests {
	...
	private static final Instant EVENT_AT = Instant.parse("2026-08-27T12:52:00Z");
	...
	void confirmedOrderHasEquivalentUtf8HtmlAndTextWithArgentineFormats() {
		RenderedEmail result = renderer.orderConfirmed(order("Ignacio", "Canguro"), store(),
			new EmailBranding("Ñandú Tienda", "#6D3CE7", "#FFFFFF",
				"https://cdn.example.com/logo.png"), EVENT_AT);

		assertThat(result.html())
			.contains("Ñandú Tienda", "Ignacio", "ORD-000014", "Canguro", "Cantidad: 1",
				"Talle: M", "Color: Azul", "$&nbsp;19.999,00", "60 y 123",
				"27/08/2026, 09:52", "background:#6D3CE7", "color:#FFFFFF",
				"src=\"https://cdn.example.com/logo.png\"", "max-width:640px")
			.doesNotContain("lookupToken", "lookup-token", "localhost", ORDER_ID.toString(), "<style>");
		assertThat(result.text())
			.contains("Ñandú Tienda", "Ignacio", "ORD-000014", "Canguro", "Cantidad: 1",
				"Talle: M", "Color: Azul", "$ 19.999,00", "Retiro en el comercio",
				"60 y 123", "27/08/2026, 09:52")
			.doesNotContain("lookupToken", "localhost");
	}
	...
	void dynamicOrderAndStoreDataCannotInjectHtml() {
		RenderedEmail result = renderer.orderConfirmed(
			...
			store("<b>dirección</b>"),
			new EmailBranding("<svg onload=bad>", "#334155", "#FFFFFF", null), EVENT_AT);

		assertThat(result.html())
			.contains("&lt;img src=x onerror=alert(1)&gt;", "&lt;script&gt;bad()&lt;/script&gt;",
				"&lt;b&gt;dirección&lt;/b&gt;", "&lt;svg onload=bad&gt;")
			.doesNotContain("<img src=x", "<script>bad()", "<b>dirección</b>", "<svg onload");
	}

	@Test
	void rejectedReceiptIncludesReasonItemsTotalExpiryAndPendingStateInBothBodies() {
		RenderedEmail result = renderer.receiptRejected(order("Ana", "Remera"), payment(), store(),
			new EmailBranding("Tienda Sur", "#8B2F45", "#FFFFFF", null));

		assertThat(result.html())
			.contains("Tienda Sur", "ORD-000014", "Remera", "$&nbsp;19.999,00",
				"Ilegible &lt;revisar&gt;", "28/08/2026, 09:56", "sigue pendiente",
				"No adjuntamos comprobantes por email")
			.doesNotContain("Ilegible <revisar>", "<img", "href=", "lookup", "private-object-key");
		assertThat(result.text())
			.contains("Tienda Sur", "ORD-000014", "Remera", "$ 19.999,00",
				"Ilegible <revisar>", "28/08/2026, 09:56", "sigue pendiente",
				"No adjuntamos comprobantes por email");
	...
	private BankTransferPayment payment() {
		return new BankTransferPayment(20L, UUID.randomUUID(), 10L, ORDER_ID, 14L, "Ana",
			new BigDecimal("19999.00"), "ARS", Instant.parse("2026-08-28T12:56:00Z"), 1,
			BankTransferStatus.REJECTED, "private-object-key", "receipt.pdf", "application/pdf",
			100L, EVENT_AT.minusSeconds(60), EVENT_AT, 7L, "Ilegible <revisar>",
			EVENT_AT, EVENT_AT, 1L);
	}

	private StoreSettings store() { return store("60 y 123"); }

	private StoreSettings store(String address) {
		return new StoreSettings("Tienda", "ARS", "America/Argentina/Buenos_Aires",
			null, null, address, "Traé tu DNI", true, "Banco", "Tienda", "TIENDA",
			null, null, null);
	}
}
