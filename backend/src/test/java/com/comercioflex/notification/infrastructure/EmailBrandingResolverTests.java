...
class EmailBrandingResolverTests {
    ...
    private final EmailBrandingResolver resolver =
    ...
    void resolvesTheCurrentTenantNameColorAndPublicHttpsLogo() {
        properties.setPublicBaseUri(
                "https://api.comercioflex.com.ar");

        when(context.currentDatabaseKey())
                .thenReturn(Optional.of("tenant_a"));

        when(tenants.findActiveByDatabaseKey("tenant_a"))
                .thenReturn(
                        Optional.of(
                                new ActiveTenant(
                                        1L,
        ...
        assertThat(result.headerColor())
                .isEqualTo("#F5E942");

        assertThat(result.headerTextColor())
                .isEqualTo("#172033");

        assertThat(result.logoUrl())
                .isEqualTo(
                        "https://api.comercioflex.com.ar/api/v1/stores/tienda-a/media/branding/logo?v="
                                + ETAG);
    }
    ...
    void tenantDatabaseKeyScopesTheLogoAndNeverUsesAnotherTenantSlug() {
        ...
        when(tenants.findActiveByDatabaseKey("tenant_b"))
                .thenReturn(
                        Optional.of(
                                new ActiveTenant(
                                        2L,
        ...
        assertThat(first.logoUrl())
                .contains("/tienda-a/")
                .doesNotContain("tienda-b");

        assertThat(second.logoUrl())
                .contains("/tienda-b/")
                .doesNotContain("tienda-a");

        assertThat(first.headerColor())
                .isNotEqualTo(
                        second.headerColor());
    }
    ...
    private StoreSettings store(
        ...
        BrandAssetReference logo =
                withLogo
                        ? new BrandAssetReference(
                                "private/branding/logo.webp",
                                "image/webp",
                                ETAG)
                        : null;
