...
public class GuestOrderService {
  ...
  public GuestOrderCreation create(CreateGuestOrderCommand rawCommand) {
    ...
    try {
      return transactionTemplate.execute(
          status -> createInsideTransaction(command, fingerprint, lookupToken, tokenHash));
    } catch (DuplicateKeyException exception) {
      return transactionTemplate.execute(
          status -> replay(command.idempotencyKey(), fingerprint, lookupToken));
    }
  }
  ...
  public GuestOrder find(UUID orderId, String lookupToken) {
    ...
    return transactionTemplate.execute(
        status -> {
          GuestOrder order =
              repository
                  .findByPublicIdAndTokenHash(orderId, tokenHash)
  ...
  private GuestOrderCreation createInsideTransaction(
      CreateGuestOrderCommand command, byte[] fingerprint, String lookupToken, byte[] tokenHash) {
  ...
  public List<com.comercioflex.shipping.domain.ShippingModels.Quote> quote(
    ...
    List<OrderItemCommand> items =
        requestedItems.stream()
            .map(i -> validateItem(i, variants))
            .sorted(Comparator.comparing(i -> i.variantId().toString()))
            .toList();
    return transactionTemplate.execute(
        s -> {
  ...
  private PricedCart priceCart(
    ...
    for (OrderItemCommand requested : requestedItems) {
      ...
      if (!variant.sellable()
          || variant
                  ...
                  .compareTo(requested.quantity())
              < 0) {
        throw new OrderUnavailableException();
      }
      BigDecimal lineTotal =
          variant.unitPrice().multiply(requested.quantity()).setScale(2, RoundingMode.HALF_UP);
      ...
      items.add(new ReservedOrderItem(variant, UUID.randomUUID(), requested.quantity(), lineTotal));
    }
    ...
    if (paymentMethod == com.comercioflex.order.domain.OrderPaymentMethod.BANK_TRANSFER) {
      ...
      discountPercentage =
          paymentPricing.bankTransferDiscountPercentage() == null
  ...
  private GuestOrderCreation replay(UUID idempotencyKey, byte[] fingerprint, String lookupToken) {
    StoredGuestOrder stored =
        repository
            ...
            .orElseThrow(
                () -> new IllegalStateException("No se pudo recuperar el pedido idempotente."));
    return replay(stored, fingerprint, lookupToken);
  }

  private GuestOrderCreation replay(
      StoredGuestOrder stored, byte[] fingerprint, String lookupToken) {
  ...
  private CreateGuestOrderCommand validate(CreateGuestOrderCommand command) {
    if (command == null || !isVersionFour(command.idempotencyKey())) {
      throw new InvalidGuestOrderException("Idempotency-Key debe ser un UUID v4 válido.");
    }
    if (command.paymentMethod() == null) {
      throw new InvalidGuestOrderException("Debe indicar un medio de pago.");
    }
    ...
    String email = requiredText(command.customerEmail(), 254, "El correo").toLowerCase(Locale.ROOT);
    String notes = optionalText(command.notes(), 1000, "Las observaciones");
    if (command.items() == null || command.items().isEmpty()) {
      throw new InvalidGuestOrderException("El pedido debe contener al menos un producto.");
    }
    if (command.items().size() > 50) {
      throw new InvalidGuestOrderException("El pedido no puede contener más de 50 productos.");
    }
    Set<UUID> variants = new HashSet<>();
    List<OrderItemCommand> items =
  ...
  private OrderItemCommand validateItem(OrderItemCommand item, Set<UUID> variants) {
  ...
  private String optionalText(String value, int maximum, String label) {
    ...
    if (value.chars().anyMatch(Character::isISOControl)) {
      throw new InvalidGuestOrderException(label + " contiene caracteres no permitidos.");
    }
  ...
  private byte[] fingerprint(CreateGuestOrderCommand command) {
    StringBuilder canonical =
        new StringBuilder()
            .append(command.customerName())
            .append('\n')
            .append(command.customerPhone())
            .append('\n')
            .append(Objects.toString(command.customerEmail(), ""))
            .append('\n')
    ...
    if (command.shipping() != null) {
      var selection = command.shipping();
      canonical.append("\nshipping:");
      ...
      if (address != null) {
        fingerprintValue(canonical, address.street());
        fingerprintValue(canonical, address.number());
        fingerprintValue(canonical, address.apartment());
        fingerprintValue(canonical, address.city());
        fingerprintValue(canonical, address.province());
        fingerprintValue(canonical, address.postalCode());
      }
    }
    command
        .items()
  ...
  private static void fingerprintValue(StringBuilder target, String value) {
