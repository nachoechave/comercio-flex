...
class OrderTransitionExecutor {
  ...
  private static final UUID PAYMENT_SYSTEM_ACTOR_ID =
  ...
  private OrderTransitionExecution execute(
    ...
    LockedAdminOrder order =
        repository.lockOrder(command.orderId()).orElseThrow(AdminOrderNotFoundException::new);
    var replay = repository.findTransition(command.idempotencyKey());
    if (replay.isPresent()) {
      AdminOrderService.requireSameTransition(replay.get(), command);
      return OrderTransitionExecution.completed(
          repository.findDetail(command.orderId()).orElseThrow(AdminOrderNotFoundException::new));
    }
    if (!verifiedPayment
    ...
    boolean completedDelivery =
        order.fulfillmentType() == com.comercioflex.order.domain.FulfillmentType.SHIPPING
            && order.status() == OrderStatus.CONFIRMED
            && command.targetStatus() == OrderStatus.COMPLETED;
    if (!completedDelivery
        && !ALLOWED.getOrDefault(order.status(), Set.of()).contains(command.targetStatus())) {
      throw new InvalidOrderTransitionException("La transición solicitada no está permitida.");
    }
    fulfillmentPolicy.beforeTransition(order, command.targetStatus());
    ...
    if (command.targetStatus() == OrderStatus.CONFIRMED) {
      ...
      requireReservations(
          expected, repository.updateReservations(order.internalId(), "ACTIVE", "CONSUMED"));
    } else if (command.targetStatus() == OrderStatus.REJECTED) {
      int expected = repository.findStockLinesForUpdate(order.internalId()).size();
      requireReservations(
          expected, repository.updateReservations(order.internalId(), "ACTIVE", "RELEASED"));
    } else if (command.targetStatus() == OrderStatus.CANCELLED) {
      int expected = moveStock(order, command, true);
      requireReservations(
          expected, repository.updateReservations(order.internalId(), "CONSUMED", "RELEASED"));
    }

    repository.updateOrderStatus(order.internalId(), order.version(), command.targetStatus());
    ...
    fulfillmentPolicy.afterTransition(order, command.targetStatus());
    return OrderTransitionExecution.completed(
        repository.findDetail(command.orderId()).orElseThrow(AdminOrderNotFoundException::new));
  }
  ...
  private void requireReservations(int expected, int changed) {
    if (expected == 0 || changed != expected) {
      throw new InvalidOrderTransitionException("Las reservas del pedido ya no están disponibles.");
    }
  }

  private UUID movementId(UUID key, UUID variantId) {
    return UUID.nameUUIDFromBytes((key + ":" + variantId).getBytes(StandardCharsets.UTF_8));
  }
