...
import com.comercioflex.radio.application.YouTubeVideoService;
...
public class RadioSiteController {
	...
	private final YouTubeVideoService videos;
	public RadioSiteController(JdbcRadioSiteRepository repository, TenantPermissionGuard permissions, YouTubeVideoService videos) { this.repository = repository; this.permissions = permissions; this.videos = videos; }
	...
	@GetMapping("/radio-site/videos")
	java.util.List<RadioSite.Video> publicVideos() { return videos.latest(repository.site(true).settings().youtubeChannelId()); }
	...
	public record SettingsInput(@Size(max=160) String heroTitle, @Size(max=300) String heroSubtitle, @Size(max=1000) String description, @Pattern(regexp="^$|https://.+") @Size(max=500) String youtubeUrl, @Size(max=100) String youtubeChannelId, @Pattern(regexp="^$|https://.+") @Size(max=500) String instagramUrl, @Pattern(regexp="^$|https://.+") @Size(max=500) String xUrl, @Pattern(regexp="^$|https://.+") @Size(max=500) String whatsappUrl) { RadioSite.Settings value() { return new RadioSite.Settings(clean(heroTitle), clean(heroSubtitle), clean(description), clean(youtubeUrl), clean(youtubeChannelId), clean(instagramUrl), clean(xUrl), clean(whatsappUrl)); } }
