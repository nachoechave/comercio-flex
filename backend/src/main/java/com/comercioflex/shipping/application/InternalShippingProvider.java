...
public class InternalShippingProvider implements ShippingProvider {
 ...
 @Override
 public List<Quote> quote(Settings settings, BigDecimal listSubtotal, BigDecimal discount,
                         String city, String postalCode) {
  List<Quote> result = new ArrayList<>();
  for (Method m : settings.methods()) {
   if (!m.active()) continue;
   BigDecimal price = switch(m.type()) {
    case PICKUP, FIXED_RATE -> m.price();
    case LOCATION_RATE, POSTAL_CODE_RATE -> m.rules().stream()
     .filter(r -> normalize(r.destination()).equals(normalize(
       m.type() == MethodType.LOCATION_RATE ? city : postalCode)))
     .map(Rule::price).findFirst().orElse(null);
   };
   if (price == null) continue;
   boolean free = m.type() != MethodType.PICKUP && settings.freeShippingThreshold() != null
   ...
   result.add(new Quote(m.id(), m.name(), m.description(), m.type(), price, free,
    listSubtotal, discount, net, net.add(price), m.pickupAddress(), m.instructions()));
  }
