...
public class ShippingQuoteController {
 ...
 public ShippingQuoteController(GuestOrderService orders) { this.orders=orders; }
 public record Request(@NotEmpty @Size(max=50) List<@Valid CreateGuestOrderItemRequest> items,
  @NotNull OrderPaymentMethod paymentMethod, @Size(max=160) String city,
  @Size(max=20) String postalCode) {}
 @PostMapping("/quote")
 ResponseEntity<List<Quote>> quote(@Valid @RequestBody Request body,jakarta.servlet.http.HttpServletRequest request) {
 ShippingAccess.requireEcommerce(request);
  return ResponseEntity.ok().cacheControl(org.springframework.http.CacheControl.noStore()).body(
   orders.quote(body.items().stream().map(i->new OrderItemCommand(i.variantId(),i.decimalQuantity())).toList(),
    body.paymentMethod(),body.city(),body.postalCode()));
 }
}
