  ...
  CredentialCipher paymentCredentialCipher(PaymentOAuthProperties properties) {
    ...
    if (properties.encryptionKey() != null && !properties.encryptionKey().isBlank()) {
      ...
      throw new PaymentOAuthException(
          "PAYMENT_CONFIGURATION_INVALID", "Falta configurar la clave de cifrado de pagos.");
    } else {
  ...
  MerchantOAuthClient merchantOAuthClient(PaymentOAuthProperties properties) {
    ...
    RestClient identityClient =
        RestClient.builder()
            ...
            .requestFactory(requestFactory)
            .build();
    return new MercadoPagoOAuthClientAdapter(oauthClient, identityClient, properties);
  }

  private void validate(PaymentOAuthProperties properties) {
    if (!properties.enabled()) return;
    if (blank(properties.clientId())
        || blank(properties.clientSecret())
      ...
      throw new PaymentOAuthException(
          "PAYMENT_CONFIGURATION_INVALID", "La configuración OAuth de Mercado Pago está incompleta.");
    }
    if (properties.environment() == com.comercioflex.payment.domain.PaymentEnvironment.PRODUCTION
      ...
      throw new PaymentOAuthException(
          "PAYMENT_CONFIGURATION_INVALID", "Producción requiere URLs HTTPS para OAuth.");
    }
  }
