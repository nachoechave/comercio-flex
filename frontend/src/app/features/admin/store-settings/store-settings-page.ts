...
export class StoreSettingsPage {
  ...
  readonly form = this.formBuilder.nonNullable.group({
    ...
    contactPhone: [
      '',
      [Validators.maxLength(40), Validators.pattern(/^$|^[+0-9][0-9 ()\-]{6,39}$/)],
    ],
      ...
      const subscription = this.api.get(slug).subscribe({
        next: (settings) => {
          this.form.setValue({
            ...
            brandTheme: settings.brandTheme ?? 'VIOLET',
          });
          this.loading.set(false);
        },
        error: () => {
          this.errorMessage.set('No pudimos cargar la configuración.');
          this.loading.set(false);
        },
      });
  ...
  save(): void {
    ...
    if (
      this.form.invalid ||
      (!value.contactPhone.trim() && !value.contactEmail.trim()) ||
      this.saving()
    ) {
    ...
    this.csrf
      ...
      .subscribe({
        next: () => this.noticeMessage.set('La tienda quedó actualizada.'),
