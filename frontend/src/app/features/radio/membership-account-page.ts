...
@Component({ selector: 'app-membership-account', imports: [CurrencyPipe, RouterLink, MembershipPaymentPanel, MembershipPaymentHistory], styleUrl: './membership.scss', template: `
 <section class="card"><h1>{{ mode === 'history' ? 'Cuotas' : mode === 'return' ? 'Estado del pago' : mode === 'plan' ? 'Mi plan' : 'Hola, ' + name() }}</h1>
 ...
 @if(mode==='history'){<app-membership-payment-history [slug]="context.slug()!"/>}
 @else if(m.currentPeriod){<app-membership-payment-panel [paid]="m.currentPeriod.accreditationStatus==='ACCREDITED'" [cancelled]="m.state==='CANCELLED'" (updated)="reload()"/>}
 }
 }
 </section>` })
