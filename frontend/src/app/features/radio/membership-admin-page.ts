...
@Component({ selector: 'app-membership-admin', imports: [CurrencyPipe, ReactiveFormsModule], styleUrl: './membership.scss', template: `
 ...
 @if (selected(); as row) {
 <section class="card"><h2>Cuotas de {{ row.identity?.firstName }}</h2><div class="table-wrap"><table><thead><tr><th>Período</th><th>Plan</th><th>Importe</th><th>Estado</th></tr></thead><tbody>
...
export class MembershipAdminPage {
 private readonly api = inject(MembershipApiService); private readonly route = inject(ActivatedRoute);
 ...
 readonly form = new FormGroup({ name: new FormControl('', { nonNullable: true, validators: [Validators.required, Validators.maxLength(120)] }), description: new FormControl('', { nonNullable: true, validators: [Validators.maxLength(2000)] }), price: new FormControl(0, { nonNullable: true, validators: [Validators.required, Validators.min(0), Validators.max(9999999999.99), Validators.pattern(/^\d+(\.\d{1,2})?$/)] }), currency: new FormControl(this.currency, { nonNullable: true }), benefits: new FormControl('', { nonNullable: true }), active: new FormControl(true, { nonNullable: true }), displayOrder: new FormControl(0, { nonNullable: true, validators: [Validators.required, Validators.min(0), Validators.max(1000000), Validators.pattern(/^\d+$/)] }) });
 constructor() { this.reloadPlans(); if (this.mode !== 'plans') this.loadMembers(); }
 ...
 edit(plan?: Plan) { this.editingId.set(plan?.publicId); this.form.reset({ name: plan?.name ?? '', description: plan?.description ?? '', price: plan?.price ?? 0, currency: plan?.currency ?? this.currency, benefits: plan?.benefits.join('\n') ?? '', active: plan?.active ?? true, displayOrder: plan?.displayOrder ?? 0 }); this.editing.set(true); }
 ...
 toggle(plan: Plan) { const { publicId, ...value } = plan; this.persist({ name: value.name, description: value.description, price: value.price, currency: value.currency, benefits: value.benefits, displayOrder: value.displayOrder, active: !value.active }, publicId); }
