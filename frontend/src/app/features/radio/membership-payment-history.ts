...
@Component({selector:'app-membership-payment-history',imports:[CurrencyPipe,DatePipe],styleUrl:'./membership.scss',template:`
 ...
 <div class="table-wrap"><table [class.member-history-table]="!membershipId()"><thead><tr><th>Período</th><th>Plan</th><th>Importe</th><th>Cuota</th><th>Pago</th><th>Fecha de pago</th>@if(membershipId()){<th>ID Mercado Pago</th><th>Revisión</th>}</tr></thead><tbody>
 @for(p of payments();track p.publicId){<tr><td data-label="Período">{{p.periodMonth}}/{{p.periodYear}}</td><td data-label="Plan">{{p.planName}}</td><td data-label="Importe">{{p.amount|currency:p.currency}}</td><td data-label="Cuota">{{p.periodStatus==='ACCREDITED'?'PAGADO':'PENDIENTE'}}</td><td data-label="Pago">{{p.providerStatus}}</td><td data-label="Fecha de pago">{{p.paidAt ? (p.paidAt|date:'dd/MM/yyyy HH:mm') : '—'}}</td>@if(membershipId()){<td>{{p.providerPaymentId}}</td><td>{{p.reviewReason || '—'}}</td>}</tr>}
