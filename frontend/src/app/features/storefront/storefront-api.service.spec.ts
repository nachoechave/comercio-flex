...
describe('StorefrontApiService', () => {
  ...
  it('loads public settings and categories for the selected store', () => {
    ...
    const settings = http.expectOne(
      '/api/v1/stores/tienda-a/settings',
    );
    ...
    const categories = http.expectOne(
      '/api/v1/stores/tienda-a/catalog/categories',
    );
  ...
  it('sends pagination and optional catalog filters', () => {
    ...
    const request = http.expectOne(
      (candidate) =>
        candidate.url ===
          '/api/v1/stores/tienda-a/catalog/products' &&
    ...
    expect(request.request.params.get('category')).toBe(
      'infantil',
    );
  ...
  it('omits empty optional filters', () => {
    ...
    const request = http.expectOne(
      '/api/v1/stores/tienda-a/catalog/products?page=0&size=24',
    );

    expect(request.request.params.has('q')).toBe(false);

    expect(
      request.request.params.has('category'),
    ).toBe(false);
  ...
  it('creates and retrieves a guest order with its private headers and token', () => {
    ...
    service
      .createOrder('tienda-a', 'key-1', body)
      .subscribe();

    const create = http.expectOne(
      '/api/v1/stores/tienda-a/orders',
    );

    expect(create.request.method).toBe('POST');

    expect(
      create.request.headers.get('Idempotency-Key'),
    ).toBe('key-1');

    expect(create.request.body).toEqual(body);

    expect(
      create.request.body.items[0].unitPrice,
    ).toBeUndefined();
    ...
    const find = http.expectOne(
      (candidate) =>
        candidate.url ===
          '/api/v1/stores/tienda-a/orders/order%2F1' &&
